...
library(ggfortify)
KM_data <- fortify(KM, surv.connect = TRUE)
...
sKM <- summary(KM, times = time_scale, extend = TRUE)
table_data <- data.frame(time = sKM$time, at_risk = sKM$n.risk, censor = sKM$n.censor, events = sKM$n.event, strata = sKM$strata)
# Reshape long
library(tidyr)
...
tds <- lapply(seq_along(tds), function(i) {
  ggplot(tds[[i]], aes(x = time, y = name, label = value)) +
...
ggsave(KM_plot, filename = "R/alessandro-gasparini.png", dpi = 600, height = 7, width = 7 / sqrt(2))
